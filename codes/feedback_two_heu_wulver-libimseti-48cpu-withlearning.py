...
def worker_loop(
    ...
    """
    UPDATED worker_loop with ML instrumentation + more robust logging.

    Key improvements vs your previous version:
      1) ALWAYS emits a result_queue payload per popped index (even bad tuple / bad idx / not-backward).
         -> prevents "holes" in ML dataset and helps parent-side aggregation.
      2) ML rows are emitted for ALL attempts, including:
         - skip_not_backward
         - bad_idx / bad_tuple
         - error
      3) Adds a compact ML per-worker summary line (always) + periodic ML samples (rate-limited).
      4) Keeps your correctness invariants:
         - strict permutation rank_to_node
         - between via rank_to_node slicing (rank-sorted)
         - changed_scores computed only over touched block [idx_v..idx_u] inclusive
         - deadline-aware exit
    """
    import time
    ...
    while True:
        ...
        # Defaults (will be included in payload even for early exits)
        u = v = None
        lo = hi = None
        w = 0.0
        mode = "none"
        ...
        # ML row (filled progressively)
        ml_row = None

        # Utility: finalize + push a payload (used for early exits too)
        def _emit_payload(*, force_mode=None, force_success=None, force_delta=None, force_changed=None, err=None):
            ...
            total_edge_processing_time_local = edge_total_time  # for local assembly only

            # Accounting for success/failure counters (only once per popped idx)
            if is_real_success:
                # count as success
                successes += 1
            ...
            if ML_ENABLE_LOGGING:
                ...
                # periodic ML samples (rate limited)
            ...
            payload = {
                ...
                "timing": {
                    ...
                    "edge_total_time": float(total_edge_processing_time_local),
                },
                "debug_flags": {
        ...
        try:
            ...
            t_btw = time.time()
            if idx_u - idx_v > 1:
                between = rank_to_node[idx_v + 1: idx_u]  # permutation -> no None
            else:
                between = []
            ...
            # emit an error payload (and ML row) so parent sees it
            _emit_payload(force_mode="error", force_success=False, force_delta=0.0, force_changed={}, err=repr(e))
            continue

        finally:
            # accumulate edge processing time into worker total timer
