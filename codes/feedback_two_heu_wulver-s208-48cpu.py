...
def worker_loop(worker_idx, num_procs,
    ...
    """
    Worker loop for func1.

    Updates:
      (A) FIX: When a move is accepted, changed_scores is collected ONLY on nodes that can
          actually be affected: interval_nodes = {u, v} ∪ between.
          This removes the int(lo)/int(hi) bug that can create partial updates.
      (B) Adds crash-only diagnostics: if any RuntimeError/Exception occurs, logs a compact
          but informative packet so you can reproduce and understand.
      (C) Keeps logging quiet: no new spam; only debug=True or crash paths log.
    """
    import time
    import os
    ...
    def _fatal_packet(title: str, **kv):
        """
        Very low-noise: only called on crash or invariant violation.
        Writes a bounded amount of info (no huge dumps).
        """
        log_message(f"[W{worker_idx:02d}][FATAL][pid={pid}] {title}", log_path)
        for k, v in kv.items():
            try:
                s = str(v)
                if len(s) > 1200:
                    s = s[:1200] + "...(truncated)"
                log_message(f"[W{worker_idx:02d}][FATAL] {k}={s}", log_path)
            except Exception:
                log_message(f"[W{worker_idx:02d}][FATAL] {k}=(unprintable)", log_path)

    # -------------------- SNAPSHOT / RANK ARRAY --------------------
    if not scores_snapshot:
        _fatal_packet("scores_snapshot is empty")
        raise RuntimeError("Empty scores_snapshot.")

    # NOTE: Your code assumes integer ranks 0..max_rank with no holes.
    # We keep that assumption but make error logs richer if violated.
    try:
        max_rank = max(scores_snapshot.values())
    except Exception as e:
        _fatal_packet("cannot compute max_rank", err=repr(e))
        raise

    if not isinstance(max_rank, int):
        # This is a hard invariant for rank_to_node indexing.
        _fatal_packet("max_rank is not int (ranks not dense integers?)", max_rank=max_rank, type=str(type(max_rank)))
        raise RuntimeError("Ranks must be integers for rank_to_node indexing.")

    rank_to_node = [None] * (max_rank + 1)

    for node, r in scores_snapshot.items():
        if not isinstance(r, int):
            _fatal_packet("non-integer rank found in scores_snapshot", node=node, rank=r, type=str(type(r)))
            raise RuntimeError("Non-integer rank in scores_snapshot.")
        if r < 0 or r > max_rank:
            _fatal_packet("rank out of bounds", node=node, rank=r, max_rank=max_rank)
            raise RuntimeError("Invalid rank in scores_snapshot.")
        if rank_to_node[r] is not None:
            _fatal_packet("duplicate rank in scores_snapshot", rank=r, node1=rank_to_node[r], node2=node)
            raise RuntimeError("Non-injective ranks in scores_snapshot.")
        rank_to_node[r] = node

    # A fast internal consistency check (cheap)
    for r in set(scores_snapshot.values()):
        if rank_to_node[r] is None:
            _fatal_packet("rank_to_node slot is None but rank exists", rank=r)
    ...
    while True:
        ...
        try:
            ...
            total_queue_wait_time += (time.time() - t0)
            _fatal_packet("edge_queue.get exception", err=repr(e))
            break
        ...
        # Defaults for crash packet context
        u = v = None
        lo = hi = None
        mode = "none"
        success = False
        ...
        try:
            ...
            if t_between > BETWEEN_WARN:
                dbg_log(f"'between' slow: u={u} v={v} len={len(between)} dt={t_between:.6f}s")

            # Occasional correctness spot-check
            if between_checks_done < BETWEEN_SAFETY_CHECK_LIMIT:
                ...
                if set(between) != set(slow_between):
                    _fatal_packet(
                        "Fast between mismatch vs slow method",
                        idx=idx, u=u, v=v, idx_u=idx_u, idx_v=idx_v,
                        fast_len=len(between), slow_len=len(slow_between),
                    )
            ...
            try:
                ...
                ext_exception += 1
                dbg_log(f"EXT_EXC u={u} v={v} err={repr(e)}")
                success = False
                delta_bw_reduction = 0.0
                mode = "extended_error"
            ...
            if t_apply_dur > APPLY_WARN:
                dbg_log(f"EXT_SLOW u={u} v={v} dt={t_apply_dur:.6f}s")
            ...
            if success:
                if delta_bw_reduction > 0.0:
                    ...
                    dbg_log(f"EXT_SUCC_NONPOS u={u} v={v} delta={delta_bw_reduction:.6g} -> reject")
                    success = False
                    delta_bw_reduction = 0.0
            ...
            if not (success and delta_bw_reduction > 0.0):
                ...
                if best_gain > 0:
                    ...
                    if debug and DEBUG_LOG_TOP_GAINS and delta_bw_reduction >= DEBUG_BIG_GAIN:
                        dbg_log(f"GREEDY_BIG u={u} v={v} delta={delta_bw_reduction:.6g} (g1={g1:.3g},g2={g2:.3g},g3={g3:.3g}) mode={mode}")
                else:
                    greedy_nonpos += 1
                    success = False
                    delta_bw_reduction = 0.0
                    ...
                    if debug:
                        ...
                        if (none_seen % max(1, DEBUG_SAMPLE_EVERY) == 0) and (dbg_edge_logs < DEBUG_MAX_EDGE_LOGS):
                            dbg_log(f"NONE_SAMPLE u={u} v={v} g1={g1:.6g} g2={g2:.6g} g3={g3:.6g} ext_mode={mode}")

            # -----------------------------------------------
            # 4) Collect rank changes (FIXED)
            #    ONLY from nodes that can actually move: {u,v} ∪ between
            #    And enforce a cheap local permutation check.
            # -----------------------------------------------
            if success and delta_bw_reduction > 0.0:
                # exact affected set
                interval_nodes = [u, v] + list(between)
                # stable-unique in O(k)
                seen = set()
                interval_nodes = [x for x in interval_nodes if (x not in seen and not seen.add(x))]

                # local permutation check
                old_ranks = [scores_base[n] for n in interval_nodes]
                new_ranks = [scores.get(n, scores_base[n]) for n in interval_nodes]

                # This MUST be true for a valid "local reorder"
                perm_ok = (sorted(old_ranks) == sorted(new_ranks)) and (len(set(new_ranks)) == len(new_ranks))

                if not perm_ok:
                    _fatal_packet(
                        "INVALID_MOVE: local rank multiset mismatch",
                        idx=idx, u=u, v=v, mode=mode, delta=delta_bw_reduction,
                        idx_u=idx_u, idx_v=idx_v,
                        lo=lo, hi=hi,
                        between_len=len(between),
                        interval_len=len(interval_nodes),
                        old_min=min(old_ranks), old_max=max(old_ranks),
                        new_min=min(new_ranks), new_max=max(new_ranks),
                        # small samples
                        old_sample=old_ranks[:20],
                        new_sample=new_ranks[:20],
                        nodes_sample=interval_nodes[:20],
                    )
                    # reject safely
                    success = False
                    delta_bw_reduction = 0.0
                    mode = f"{mode}_perm_fail"
                    changed_scores = {}
                else:
                    # Build minimal changed_scores on those nodes
                    ...
                    # If "success" but no changes -> reject (keeps invariant)
                    if not changed_scores:
                        reject_changed_empty += 1
                        dbg_log(f"CHANGED_EMPTY u={u} v={v} delta={delta_bw_reduction:.6g} mode={mode} -> reject")
                        success = False
                        delta_bw_reduction = 0.0
            ...
            # Rich log (this is what you asked for)
            edge_total_time = time.time() - edge_start
            total_edge_processing_time += edge_total_time
            unhandled_errors += 1
            failures += 1
            ...
            _fatal_packet(
                "RuntimeError inside worker edge-processing",
                err=repr(e),
                idx=idx,
                u=u, v=v,
                mode=mode,
                delta=delta_bw_reduction,
                lo=lo, hi=hi,
                idx_u=(scores_base[u] if u is not None and u in scores_base else None),
                idx_v=(scores_base[v] if v is not None and v in scores_base else None),
                between_len=len(between) if between is not None else None,
                changed_len=len(changed_scores) if changed_scores is not None else None,
                t_between=t_between,
                t_apply=t_apply_dur,
                t_greedy=greedy_dur,
                t_edge=edge_total_time,
            )
            ...
            # Unexpected exception: also log rich packet
            edge_total_time = time.time() - edge_start
            total_edge_processing_time += edge_total_time
            unhandled_errors += 1
            failures += 1
            ...
            _fatal_packet(
                "Unhandled Exception inside worker edge-processing",
    ...
    if debug:
        log_message(
            ...
            f"t_queue={total_queue_wait_time:.3f}s t_apply={total_apply_strategy_time:.3f}s t_greedy={total_greedy_time:.3f}s "
            f"t_edges={total_edge_processing_time:.3f}s dbg_edge_logs={dbg_edge_logs}",
            log_path
        )
...
def refine_ranking_parallel_dynamic(
    ...
    """
    Quiet version: logs only high-signal events:
      - START / END
      - IMPROVE (only if something applied)
      - NEW_BEST (only if best improves, includes save)
      - STOP reasons
      - WARN if workers die/miss done or we had to terminate
      - ERROR/FATAL packets if BW increases / sanity mismatch / invariant breaks

    SAFETY:
      - If an accepted change increases BW (even once), raise RuntimeError.
      - Controller validates local-permutation + injective ranks for any accepted changed_scores.
    """
    import time
    import os
    import multiprocessing as mp
    ...
    # ------------------------------
    # Crash-only diagnostics (bounded)
    # ------------------------------
    def _fatal_packet(title: str, **kv):
        log_message(f"[FUNC1][FATAL][pid={pid_main}] {title}", log_path)
        for k, v in kv.items():
            try:
                s = str(v)
                if len(s) > 1600:
                    s = s[:1600] + "...(truncated)"
                log_message(f"[FUNC1][FATAL] {k}={s}", log_path)
            except Exception:
                log_message(f"[FUNC1][FATAL] {k}=(unprintable)", log_path)

    # ------------------------------
    # Helpers: local validation / apply
    # ------------------------------
    def _validate_changed_scores_local(scores_before, changed_scores, ctx_tag=""):
        """
        Validates that within the locally-changed region:
          - new ranks are injective
          - rank multiset is preserved (permutation of old ranks)
        This catches partial updates and inconsistent worker returns.
        """
        nodes = list(changed_scores.keys())
        if len(nodes) < 2:
            return True, ""

        old_ranks = [scores_before[n] for n in nodes]
        new_ranks = [changed_scores[n] for n in nodes]

        if len(set(new_ranks)) != len(new_ranks):
            return False, f"{ctx_tag} non-injective new ranks inside changed_scores"

        if sorted(old_ranks) != sorted(new_ranks):
            return False, f"{ctx_tag} rank multiset mismatch (not a permutation)"

        return True, ""

    def _apply_changed_scores(scores_live, changed_scores):
        for node, new_r in changed_scores.items():
            scores_live[node] = new_r
    ...
    while True:
        ...
        for gi in S_indices:
            ...
            if ru > rv:
                edges_for_dp.append((u, v, w, rv, ru))  # (u,v,w,lo,hi) kept for compatibility only
        ...
        if num_edges_batch <= int(SERIAL_BELOW_EDGES):
            ...
            for i in range(num_edges_batch):
                ...
                idx_u = scores_snapshot[u]
                idx_v = scores_snapshot[v]

                # build between (tiny batch => ok to do slow)
                between = [n for n, r in scores_snapshot.items() if idx_v < r < idx_u]
                ...
                # ---------------- FIX: changed_scores only on {u,v} ∪ between
                interval_nodes = [u, v] + list(between)
                seen = set()
                interval_nodes = [x for x in interval_nodes if (x not in seen and not seen.add(x))]
                ...
                changed_scores = {}
                for node in interval_nodes:
                    r_old = scores_snapshot[node]
                    r_new = scores_local.get(node, r_old)
                    if r_new != r_old:
                        changed_scores[node] = r_new
                ...
                ok, why = _validate_changed_scores_local(scores_snapshot, changed_scores, ctx_tag="SERIAL")
                if not ok:
                    _fatal_packet(
                        "SERIAL invalid changed_scores (local invariant broken)",
                        why=why,
                        ...
                        delta=delta,
                        between_len=len(between),
                        changed_len=len(changed_scores),
                        nodes_sample=list(changed_scores.keys())[:20],
                        old_ranks_sample=[scores_snapshot[n] for n in list(changed_scores.keys())[:20]],
                        new_ranks_sample=[changed_scores[n] for n in list(changed_scores.keys())[:20]],
                    )
                    raise RuntimeError("SERIAL accept produced invalid changed_scores.")

                # optional recompute safety
                if recompute_left > 0:
                    bw_before = total_weight - float(compute_forward_weight(edges, scores))
                    scores_test = scores.copy()
                    _apply_changed_scores(scores_test, changed_scores)
                    bw_after = total_weight - float(compute_forward_weight(edges, scores_test))
                    if bw_after > bw_before + BW_INCREASE_TOL:
                        _fatal_packet(
                            "BW increased on SERIAL ACCEPT (recomputed)",
                            batch=dp_batches_done,
                            edge=(u, v),
                            mode=mode,
                            ...
                            bw_before=bw_before,
                            bw_after=bw_after,
                            bw_increase=bw_after - bw_before,
                            between_len=len(between),
                            changed_len=len(changed_scores),
                        )
                        raise RuntimeError("Backward weight increased after SERIAL accept.")
                    recompute_left -= 1

                bw_before_tracked = BW_tracked
                _apply_changed_scores(scores, changed_scores)
                BW_tracked -= delta

                if BW_tracked > bw_before_tracked + BW_INCREASE_TOL:
                    _fatal_packet(
                        "BW_tracked increased after SERIAL ACCEPT",
                        batch=dp_batches_done,
                        edge=(u, v),
                        mode=mode,
                        delta=delta,
                        BW_before=bw_before_tracked,
                        BW_after=BW_tracked,
                    )
                    raise RuntimeError("BW_tracked increased after SERIAL accept.")
            ...
            if BW_CHECK_EVERY_ROUND > 0 and (round_idx % BW_CHECK_EVERY_ROUND == 1):
                ...
                if abs(bw_recompute - BW_tracked) > BW_MISMATCH_TOL:
                    _fatal_packet(
                        "SANITY_MISMATCH (SERIAL path)",
                        round=round_idx,
                        batch=dp_batches_done,
                        BW_recompute=bw_recompute,
                        BW_tracked=BW_tracked,
                        diff=bw_recompute - BW_tracked,
                    )
        ...
        # ------------------------------
        # Apply ACCEPTED results + STRICT checks
        # ------------------------------
        applied_edges_round = 0
        total_bw_reduct_round = 0.0
        recompute_left = int(DEBUG_RECOMP_ACCEPTS_PER_BATCH) if debug else 0
        ...
        for r in results:
            ...
            # Validate local changed_scores against THIS BATCH snapshot (important!)
            ok, why = _validate_changed_scores_local(scores_snapshot, changed_scores, ctx_tag="PARALLEL")
            if not ok:
                _fatal_packet(
                    "PARALLEL invalid changed_scores from worker",
                    why=why,
                    batch=dp_batches_done,
                    worker=r.get("worker_idx", None),
                    edge=(r.get("u", None), r.get("v", None)),
                    mode=r.get("mode", "?"),
                    delta=delta,
                ...
                raise RuntimeError("Invalid changed_scores returned by worker (controller rejected).")
            ...
            if recompute_left > 0:
                ...
                if bw_after > bw_before + BW_INCREASE_TOL:
                    _fatal_packet(
                        "BW increased on PARALLEL ACCEPT (recomputed)",
                        batch=dp_batches_done,
                        worker=r.get("worker_idx", None),
                        edge=(r.get("u", None), r.get("v", None)),
                        mode=r.get("mode", "?"),
                    ...
                    raise RuntimeError("Backward weight increased after PARALLEL accept (forbidden).")
            ...
            if BW_tracked > bw_before_tracked + BW_INCREASE_TOL:
                _fatal_packet(
                    "BW_tracked increased after PARALLEL ACCEPT",
                    batch=dp_batches_done,
                    worker=r.get("worker_idx", None),
                    edge=(r.get("u", None), r.get("v", None)),
                    mode=r.get("mode", "?"),
                ...
                raise RuntimeError("BW_tracked increased after PARALLEL accept (forbidden).")
        ...
        # ------------------------------
        # Update S: remove assessed edges from this batch
        # ------------------------------
        assessed_set = set(selected_global_indices)
        S_indices = [gi for gi in S_indices if gi not in assessed_set]
        S_indices = filter_S_keep_backward(scores, S_indices)
...
graph_file_path = os.path.join(base_dir, "s208.d")
...
final_ranking_path = hybrid_refine_func1_func2(
    ...
    RUN_FOR_MINUTES=600.0,          # 10 hours
