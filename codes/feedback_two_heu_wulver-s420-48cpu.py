...
def worker_loop(worker_idx, num_procs,
                ...
                QUEUE_GET_TIMEOUT_SEC: float = 1.0,   # periodic wakeup to allow robust exit (even if sentinels late)
               ):
    """
    IMPORTANT FIX (load balance):
      - Use BLOCKING edge_queue.get() with sentinels (idx=None) instead of get_nowait().
      - This prevents most workers from exiting early due to Queue-feeder timing and fixes
        the “one worker does all the jobs” symptom.

    Contract expected by main:
      - main enqueues integer indices, and then enqueues one None sentinel per worker.
      - worker breaks when it receives None.
      - worker posts a final {"done": True, ...} summary dict.

    Logging remains low-noise:
      - debug=True enables a small number of per-edge dbg logs.
      - one per-worker summary line is ALWAYS sent in result_queue (and optionally logged by main).
    """
    import time
    ...
    # -------------------- NEW: queue lifecycle stats --------------------
    sentinels_seen = 0
    get_timeouts = 0
    first_item_ts = None
    last_item_ts = None

    # (Optional) log that worker started (very low noise: only in debug)
    if debug:
    ...
    while True:
        ...
        #    - idx=None is sentinel => stop
        # -----------------------------------------------
        t0 = time.time()
        try:
            idx = edge_queue.get(timeout=QUEUE_GET_TIMEOUT_SEC)
        except queue.Empty:
            # Do NOT exit: allow time for feeder/sentinels; just record and keep waiting
            total_queue_wait_time += (time.time() - t0)
            get_timeouts += 1
            continue
        except Exception:
            # Conservative: break (main will see early-done distribution)
            total_queue_wait_time += (time.time() - t0)
            break
        ...
        try:
            ...
            if success and delta_bw_reduction > 0.0:
                ...
                hi_i = int(hi) if hi is not None else -1
                if lo_i < 0:
                    lo_i = 0
                if hi_i > max_rank:
                    hi_i = max_rank

                interval_nodes = rank_to_node[lo_i:hi_i + 1] if hi_i >= lo_i else []
                ...
                if not changed_scores:
                    ...
                    dbg_log(f"CHANGED_EMPTY u={u} v={v} delta={delta_bw_reduction:.6g} mode={mode} lo={lo_i} hi={hi_i} -> reject")
                    success = False
                    delta_bw_reduction = 0.0
    ...
    # One compact debug line here is useful for diagnosing imbalance quickly.
    # (Safe even if many workers: one line per worker only, and only when debug=True.)
    if debug:
        log_message(
...
def refine_ranking_parallel_dynamic(
    ...
    Quiet version: logs only high-signal events:
      ...
      - ERROR if BW increases / sanity mismatch
    ...
    def _cleanup_workers(workers, reason_tag: str):
        """
        Join briefly; terminate anything still alive; join again.
        Returns number terminated.
        """
        terminated = 0
        for p in workers:
            try:
    ...
    while True:
        ...
        if num_edges_batch <= int(SERIAL_BELOW_EDGES):
            ...
            for i in range(num_edges_batch):
                # Call worker_loop logic in-process would require refactor.
                # Instead, reuse the same primitives directly (extended+greedy)
                # by invoking worker_loop with a fake queue would be messy.
                # So we do a minimal faithful version here using the same helpers.
                ...
                # build between (slow but tiny batch => ok)
                ...
                if not success:
                    try:
                        # compute_all_gains_local signature may vary; simplest: try with log_path then without
                        try:
                        ...
                        if best_gain > 0.0:
                            success = True
                            delta = best_gain
                            # apply the corresponding move on a copy
                            scores_local = scores_snapshot.copy()
                            if best_gain == g1:
                ...
                # collect changed_scores only within [lo,hi]
                lo_i = int(lo) if lo is not None else 0
                hi_i = int(hi) if hi is not None else -1
                if lo_i < 0:
                    lo_i = 0
                # hi clamp needs max rank in scores_snapshot
                max_rank = max(scores_snapshot.values())
                if hi_i > max_rank:
                    hi_i = max_rank

                # slow interval scan (tiny batch)
                for node, r_old in scores_snapshot.items():
                    if lo_i <= r_old <= hi_i:
                ...
                if recompute_left > 0:
                    ...
                    if bw_after > bw_before + BW_INCREASE_TOL:
                        log_message(
                            f"[FUNC1][ERROR] BW increased on ACCEPT (recomputed)! before={bw_before:.6f} after={bw_after:.6f} "
                            f"(+{bw_after - bw_before:.6g}) delta_reported={delta:.6g}",
                            log_path
                        )
                        raise RuntimeError("Backward weight increased after applying an accepted change (forbidden).")
                    recompute_left -= 1
                ...
                if BW_tracked > bw_before_tracked + BW_INCREASE_TOL:
                    log_message(
                        f"[FUNC1][ERROR] BW_tracked increased after ACCEPT! before={bw_before_tracked:.6f} after={BW_tracked:.6f} delta={delta:.6g}",
                        log_path
                    )
                    raise RuntimeError("BW_tracked increased after applying an accepted change (forbidden).")
            ...
            if BW_CHECK_EVERY_ROUND > 0 and (round_idx % BW_CHECK_EVERY_ROUND == 1):
                ...
                if abs(bw_recompute - BW_tracked) > BW_MISMATCH_TOL:
                    log_message(
                        f"[FUNC1][ERROR] SANITY_MISMATCH round={round_idx} BW_recompute={bw_recompute:.6f} BW_tracked={BW_tracked:.6f}",
                        log_path
                    )
        ...
        # Critical: if we broke due to deadline, we must cleanup aggressively
        ...
        # If deadline triggered, stop the whole function (high-signal, prevents spam)
        ...
        for r in results:
            ...
            if recompute_left > 0:
                ...
                if bw_after > bw_before + BW_INCREASE_TOL:
                    log_message(
                        f"[FUNC1][ERROR] BW increased on ACCEPT (recomputed)! before={bw_before:.6f} after={bw_after:.6f} "
                        f"(+{bw_after - bw_before:.6g}) delta_reported={delta:.6g} mode={r.get('mode','?')} interval={r.get('interval',None)}",
                        log_path
                    )
                    raise RuntimeError("Backward weight increased after applying an accepted change (forbidden).")
            ...
            if BW_tracked > bw_before_tracked + BW_INCREASE_TOL:
                log_message(
                    f"[FUNC1][ERROR] BW_tracked increased after ACCEPT! before={bw_before_tracked:.6f} after={BW_tracked:.6f} "
                    f"delta={delta:.6g} mode={r.get('mode','?')} interval={r.get('interval',None)}",
                    log_path
                )
                raise RuntimeError("BW_tracked increased after applying an accepted change (forbidden).")
        ...
        # High-signal: log only when applied > 0
        if applied_edges_round > 0:
            log_message(
                f"[FUNC1] IMPROVE batch={dp_batches_done} applied={applied_edges_round}/{num_edges_batch} "
                f"| dBW={-total_bw_reduct_round:.6f} | BW_now={BW_tracked:.2f} best={best_BW:.2f} | S={len(S_indices)}",
        ...
        # Global BW sanity check sometimes (log only on error)
        if BW_CHECK_EVERY_ROUND > 0 and (round_idx % BW_CHECK_EVERY_ROUND == 1):
            fw_recompute = compute_forward_weight(edges, scores)
            bw_recompute = total_weight - float(fw_recompute)
            if abs(bw_recompute - BW_tracked) > BW_MISMATCH_TOL:
        ...
        # No-improvement stop (high-signal)
        if (time.time() - last_improvement_time) >= NO_IMPROVEMENT_TIME_LIMIT_SEC:
            log_message(
                f"[FUNC1] STOP no_improve_for={time.time() - last_improvement_time:.1f}s limit={NO_IMPROVEMENT_TIME_LIMIT_SEC:.0f}s",
                log_path
...
graph_file_path = os.path.join(base_dir, "s420.d")
