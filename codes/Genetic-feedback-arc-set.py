...
# ==========================================
# 3. Main Genetic Algorithm Class
# ==========================================
class WFASParallelSolver:
    def __init__(self, u_arr, v_arr, w_arr, node_to_index, index_to_node, output_dir, pop_size=70):
        ...
        # REQ: "ranking file should have genetic-feedback in it"
        self.unified_csv = os.path.join(output_dir, "unified_rankings_genetic_feedback.csv")
        self.log_file = os.path.join(output_dir, f"run_genetic_feedback_{self.run_id}.log")
    ...
    def append_result(self, generation, fitness, perm):
        ...
        # Added timestamp to CSV for paper analysis
    ...
    def solve(self, max_seconds, stagnation_limit):
        ...
        self.logger.info(f"Max Time: {max_seconds}s, Stagnation Limit: {stagnation_limit}")
        
        print(f"Generating initial population...")
        init_start = time.time()
        
        init_perms = [self.generate_random_perm() for _ in range(self.pop_size)]
        ...
        self.logger.info(f"Initialization Complete. Duration: {init_duration:.2f}s. Initial Best: {best_global['fit']}")
        ...
        self.logger.info(f"Starting Worker Pool with {cpu_count} CPUs.")
        ...
        try:
            while True:
                ...
                if total_elapsed > max_seconds:
                    self.logger.info("STOPPING: Max time limit reached.")
                    print("Time limit reached.")
                    break
                if rounds_stag >= stagnation_limit:
                    self.logger.info("STOPPING: Stagnation limit reached.")
                ...
                if not tasks:
                    self.logger.info("STOPPING: Population converged (no new pairs).")
                    break
                
                # Execute Generation
                results = pool.map(worker_task, tasks, chunksize=max(1, len(tasks)//(cpu_count*4)))
                ...
                log_msg = f"Gen {rounds} | Duration: {gen_duration:.2f}s | Pairs: {len(tasks)} | Best: {current_best['fit']}"
                
                if current_best['fit'] > best_global['fit']:
                    best_global = current_best
                    rounds_stag = 0
                    self.logger.info(f"{log_msg} | **NEW BEST FOUND**")
                    ...
                    rounds_stag += 1
                    self.logger.info(f"{log_msg} | Stagnation: {rounds_stag}")
                    print(f"Gen {rounds} | Best: {best_global['fit']} | Stag: {rounds_stag} (in {gen_duration:.2f}s)")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(INPUT_PATH):
        ...
        output_dir = "."
        # sys.exit(1) # Commented out for local testing if file missing
    else:
