...
def parallel_refine_largest_scc_intervals(
    ...
    Update in THIS VERSION (fairness + avoid batches=0):
      (A) Build out_edges and use it in the backward-edge filter so the filter matches GLOBAL blocks.
      (B) Add rejection counters + a compact build-failure line (diagnostic).
      (C) Add a small fallback: after a few empty builds, force-build 1 block to give func2 a chance.
    """
    ...
    # ---- build out_edges ONCE (used by fair backward-edge filter)
    out_edges = {}
    ...
    # ---- FAIR backward-edge filter (UPDATED): pass out_edges
    def _rank_interval_has_backward_edge(
        edges_local,
        scores_loc,
        r_low,
        r_high,
        ...
        try:
            r_low_f = float(r_low)
            r_high_f = float(r_high)
        except Exception:
            r_low_f = r_low
            r_high_f = r_high
        if r_low_f > r_high_f:
            r_low_f, r_high_f = r_high_f, r_low_f

        if out_edges is not None:
            block_nodes = []
            for n, r in scores_loc.items():
                if r_low_f <= r <= r_high_f:
            ...
            if len(block_nodes) > int(sample_k):
                sample_nodes = _rnd.sample(block_nodes, int(sample_k))
                block_set = set(block_nodes)
            else:
                sample_nodes = block_nodes
                block_set = set(block_nodes)

            for u in sample_nodes:
                ru = scores_loc[u]
                for (v, _w) in out_edges.get(u, []):
                    if v in block_set and ru > scores_loc[v]:
                        return True
            return False

        # fallback: original SCC-local edge scan
        for (u, v, _w) in edges_local:
            ru = scores_loc[u]
            rv = scores_loc[v]
            if r_low_f <= ru <= r_high_f and r_low_f <= rv <= r_high_f and ru > rv:
                return True
        return False
    ...
    while True:
        ...
        # --- rejection counters (NEW: diagnosable build failures)
        rej_no_bw = 0
        rej_cooldown = 0
        rej_overlap = 0
        rej_too_small = 0
        ...
        while len(batch_blocks) < target_blocks and attempts < max_attempts:
            ...
            # FAIR quick backward filter (NEW): use out_edges over the block ranks
            # Keep edges_in_scc[sid] as fallback/metadata only.
            if not _rank_interval_has_backward_edge(
                edges_in_scc[sid],
                snapshot_scores,
                r_low,
                r_high,
                out_edges=out_edges,
                sample_k=250,
                seed=(batch_idx + 1) * 100000 + attempts,
            ):
                rej_no_bw += 1
                continue
        ...
        # ---- NEW: if build fails, log why (compact) + small fallback after a few fails
        if not batch_blocks:
            empty_build_streak += 1
            _imp(
                f"[BUILD_FAIL] empty_streak={empty_build_streak} attempts={attempts}/{max_attempts} "
                f"target_blocks={target_blocks} rej(no_bw/cool/ovr/small/badw/oth)="
                f"{rej_no_bw}/{rej_cooldown}/{rej_overlap}/{rej_too_small}/{rej_bad_width}/{rej_other}"
            )

            # Fallback: after a few empty builds, force-build ONE random global block
            # (still safe: monotonicity check will reject harmful moves)
            if empty_build_streak >= 3 and n_nodes >= 2:
                if _time_left_sec() >= 1.0:
                    size_target = max(int(min_block_abs), min(int(max_block_abs or n_nodes), max(100, n_nodes // 100)))
                    gL = random.randint(0, max(0, n_nodes - 2))
                    gR = min(n_nodes, gL + int(size_target))
                    if gR - gL >= 2:
                        r_low = float(global_ranks_sorted[gL])
                        r_high = float(global_ranks_sorted[gR - 1])
                        batch_blocks = [{
                            "seed_sid": random.choice(active_sids),
                            "r_low": r_low,
                            "r_high": r_high,
                            "block_nodes": global_nodes_sorted[gL:gR],
                        }]
                        used_rank_ranges = [(r_low, r_high)]
                        _imp(f"[BUILD_FALLBACK] forced 1 block | size={gR-gL} ranks=[{r_low:.1f},{r_high:.1f}]")
                        empty_build_streak = 0
                        # proceed to execute batch
                    else:
                        continue
                else:
                    continue
        ...
        for i, blk in enumerate(batch_blocks):
            dp_cap_for_this_block = int(dp_exact_max_size) if (i in dp_enabled_ids) else 0
            worker_args.append((
                f"b{batch_idx}_{i}",
                blk["block_nodes"],
                blk["r_low"],
...
def hybrid_refine_func1_func2(
    ...
    """
    UPDATED (func2 fairness + measurable time share):

    Key changes:
      1) Enforce a real time budget for func2 (so func1 can't eat the whole run).
         We reserve a fraction of total time for func2 (internal policy; no new parameter).
      2) Run func2 not only "on empty", but also periodically if budget remains.
      3) Pass a derived NO_IMPROVEMENT_TIME_LIMIT_SEC into func1 so it rotates phases
         (prevents long stalls in func1 from starving func2).

    Logging remains low-noise:
      - START / STOP / END
      - phase lines only on improvement or NEW_BEST
      - sparse heartbeat (default 6 hours)
    """
    import os
    import time
    import shutil
    ...
    # ------------------------------
    # Resolve minutes stop (alias) + default h_hours behavior
    # ------------------------------
    ...
    try:
        RUN_FOR_MINUTES = float(RUN_FOR_MINUTES)
    except Exception:
        RUN_FOR_MINUTES = float(h_hours) * 60.0
    ...
    # ------------------------------
    start_ts = time.time()
    total_run_sec = float(RUN_FOR_MINUTES) * 60.0
    global_deadline = start_ts + total_run_sec
    ...
    def _hours_left():
        return max(1e-6, _sec_left() / 3600.0)
    ...
    # ------------------------------
    # func1 stall policy (derived) to avoid starving func2
    # ------------------------------
    base_no_improve_sec = max(15.0, min(600.0, 0.10 * total_run_sec))

    def _func1_no_improve_limit_sec():
        # cap by remaining time so it rotates near the end
        sec_left = _sec_left()
        cap_by_remaining = max(15.0, 0.25 * sec_left)
        return float(min(base_no_improve_sec, cap_by_remaining))

    # ------------------------------
    # NEW: func2 fairness budget (internal; no new user param)
    # ------------------------------
    # Reserve at least 20% of total time (clamped) for func2 so it's measurable.
    func2_budget_sec = 0.20 * total_run_sec
    func2_budget_sec = max(60.0, min(func2_budget_sec, 0.60 * total_run_sec))  # clamp [1 min, 60%]
    func2_spent_sec = 0.0

    # Periodic func2 tick even when cycle isn't empty (if budget remains)
    FUNC2_PERIODIC_SEC = max(30.0, min(300.0, 0.02 * total_run_sec))  # 30s..5min
    last_func2_tick_ts = start_ts

    def _func2_budget_left():
        return max(0.0, float(func2_budget_sec) - float(func2_spent_sec))

    def _func1_call_deadline():
        # Keep func2_budget_left() seconds for func2 by capping func1 calls.
        return float(global_deadline - _func2_budget_left())

    def _should_run_func2_periodic():
        if _func2_budget_left() <= 0.0:
            return False
        return (time.time() - last_func2_tick_ts) >= float(FUNC2_PERIODIC_SEC)

    # ------------------------------
    # Logging signal helper
    # ------------------------------
    ...
    def _phase_log_if_signal(phase_idx, phase_name, t0, bw_before, bw_after, extra=""):
        ...
        if is_new_best:
            ...
            heartbeat(
                f"[HYBRID] heartbeat | phase={phase_idx:04d} | "
                f"func1_runs={func1_runs} func2_runs={func2_runs} | "
                f"BW={bw_after:.6f} bestBW={best_bw_seen:.6f} | "
                f"sec_left={_sec_left():.1f} func2_budget_left={_func2_budget_left():.1f}"
            )
    ...
    log_message(
        ...
        f"FUNC2_ON_EMPTY_SEC={FUNC2_ON_EMPTY_SEC} | "
        f"func2_budget_sec={func2_budget_sec:.1f} func2_periodic_sec={FUNC2_PERIODIC_SEC:.1f} | "
        f"func1_stall_base_sec={base_no_improve_sec:.1f}",
        log_path
    )
    ...
    # =====================================================
    # PHASE 1: func1 GLOBAL (initial) with func1 budget cap
    # =====================================================
    phase_idx += 1
    func1_runs += 1
    t0 = time.time()
    ...
    _call_with_optional_deadline(
        ...
        dict(
            ...
            MAX_DP_BATCHES_PER_CALL=None,
            NO_IMPROVEMENT_TIME_LIMIT_SEC=_func1_no_improve_limit_sec(),
        ),
        deadline_ts=min(global_deadline, _func1_call_deadline()),
    )
    ...
    try:
        while True:
            ...
            # -------------------------------------------------
            # Decide if we should give func2 time now (fairness tick)
            # -------------------------------------------------
            run_func2_now = False
            func2_deadline = None

            # 1) If cycle is empty (your old trigger)
            ...
            # 2) Or periodic tick (even if cycle has items), if budget remains
            if (not run_func2_now) and _should_run_func2_periodic():
                run_func2_now = True

            if run_func2_now:
                # bounded func2 run (only if enough time remains AND budget remains)
                if (_sec_left() >= float(FUNC2_ON_EMPTY_MIN_SEC_LEFT)) and (_func2_budget_left() > 0.0):
                    phase_idx += 1
                    func2_runs += 1
                    t0 = time.time()
                    _, _, bw_before2 = bw_from_ranking_path(current_input_path)

                    # give func2 a slice of time, but never exceed budget_left or global_deadline
                    slice_sec = float(FUNC2_ON_EMPTY_SEC)
                    slice_sec = max(5.0, slice_sec)
                    slice_sec = min(slice_sec, _func2_budget_left(), _sec_left())
                    func2_deadline = min(global_deadline, time.time() + slice_sec)
                    ...
                    _call_with_optional_deadline(
                        ...
                        deadline_ts=func2_deadline,
                    )

                    dt2 = time.time() - t0
                    func2_spent_sec += dt2
                    last_func2_tick_ts = time.time()

                    _, _, bw_after2 = bw_from_ranking_path(output_ranking_path)
                    _phase_log_if_signal(
                        phase_idx,
                        f"func2_tick_{func2_runs}",
                        t0,
                        bw_before2,
                        bw_after2,
                        extra=f"dt={dt2:.2f}s budget_left={_func2_budget_left():.1f}s"
                    )
                    current_input_path = output_ranking_path

                    if _stop_if_needed("after_func2_tick"):
                        break

                    # refresh after func2
                    scores_now, _, bw_now = bw_from_ranking_path(current_input_path)

                # refill if cycle is empty/dead (keep old behavior)
                if (not cycle_remaining) or (_count_backward_in_pairs(scores_now, cycle_remaining) == 0):
                    cycle_id += 1
                    ...
                    if len(cycle_remaining) == 0:
                        heartbeat(
                            ...
                            f"BW={bw_now:.6f} bestBW={best_bw_seen:.6f} | "
                            f"sec_left={_sec_left():.1f} func2_budget_left={_func2_budget_left():.1f}"
                        )
            ...
            # -------------------------------------------------
            # func1 TARGETED (budget-capped deadline so func2 can't be starved)
            # -------------------------------------------------
            if _stop_if_needed("before_func1_targeted"):
                break

            # If func2 budget remaining is positive, cap func1 calls to preserve it.
            func1_deadline = min(global_deadline, _func1_call_deadline())
            if time.time() >= func1_deadline:
                # We've reached the func1 cap; from here on only func2 gets time
                if _func2_budget_left() > 0.0 and _sec_left() > 0.0:
                    # force func2 tick next loop
                    last_func2_tick_ts = 0.0
                    continue
                else:
                    break
            ...
            if remaining_pairs_unassessed is not None:
                ...
                # safest fallback: force func2 soon if return contract unknown
            ...
            _phase_log_if_signal(
                ...
                extra=(
                    ...
                    f"dp_batches={int(FUNC1_DP_BATCHES_PER_CALL)} "
                    f"no_improve_sec={_func1_no_improve_limit_sec():.1f} "
                    f"func2_budget_left={_func2_budget_left():.1f}"
                )
            )
            current_input_path = output_ranking_path
            ...
            # -------------------------------------------------
            # periodic GLOBAL refresh (forced) — budget-capped
            # -------------------------------------------------
            if func1_full_every > 0 and (func1_runs % func1_full_every == 0):
                ...
                _phase_log_if_signal(
                    ...
                    bw_before_p,
                    bw_after_p,
                    extra=f"mode=GLOBAL(forced) func2_budget_left={_func2_budget_left():.1f}"
                )
        ...
        log_message(
            ...
            f"cycle_id={cycle_id} bestBW={best_bw_seen:.6f} func2_spent={func2_spent_sec:.1f}s "
            f"| elapsed_h={(time.time()-start_ts)/3600.0:.3f}",
            log_path
        )
        return output_ranking_path
...
graph_file_path = os.path.join(base_dir, "s953.d")
