...
def hybrid_refine_func1_func2(
    ...
    # ---------------- core knobs ----------------
    RUN_FOR_MINUTES: float = 60.0,     # <-- the ONLY wall-clock budget knob now
    ...
    NEVER_STOP: bool = True,          # OK to leave True; we still enforce the hard stop via RUN_FOR_MINUTES
    ...
    FUNC1_DP_BATCHES_PER_CALL: int = 20,      # targeted DP batches per func1 call

    # ---------------- NEW: stall policy derived from total time ----------------
    # No extra user-facing parameter. This is computed from RUN_FOR_MINUTES.
):
    """
    Hybrid loop (func2 ↔ func1 targeted) + optional WMSF seed/escape.

    Changes vs your old version:
      1) Only ONE wall-clock knob: RUN_FOR_MINUTES.
      2) func1 "stall" (no-improve cutoff) is derived from RUN_FOR_MINUTES (and shrinks near the end),
         and is passed into refine_ranking_parallel_dynamic via NO_IMPROVEMENT_TIME_LIMIT_SEC.
    ...
    # ------------------------------
    # Normalize RUN_FOR_MINUTES
    # ------------------------------
    try:
        RUN_FOR_MINUTES = float(RUN_FOR_MINUTES)
    except Exception:
        RUN_FOR_MINUTES = 60.0
    ...
    # ------------------------------
    # func1 stall policy derived from total time
    # ------------------------------
    # Base stall limit = 10% of total time, clamped to [15s, 600s].
    # Additionally, near the end we shrink it so phases can still rotate.
    base_no_improve_sec = max(15.0, min(600.0, 0.10 * total_run_sec))

    def _func1_no_improve_limit_sec():
        # Never larger than base, and never larger than 25% of remaining time.
        # Also keep a minimum so it doesn't thrash.
        sec_left = _sec_left()
    ...
    def _stop_if_needed(where: str):
        if _time_exceeded():
            log_message(
                ...
                f"RUN_FOR_MINUTES={RUN_FOR_MINUTES} NEVER_STOP={NEVER_STOP}",
                log_path
            )
            return True
        return False
    ...
    def _phase_log_if_signal(phase_idx, phase_name, t0, bw_before, bw_after, extra=""):
        ...
        is_improve = (bw_before - bw_after) > float(EPS_BW_IMPROVEMENT)
        is_new_best = bw_after + float(EPS_BW_IMPROVEMENT) < best_bw_seen
    ...
    log_message(
        ...
        f"NEVER_STOP={NEVER_STOP} | output={output_ranking_path} | "
    ...
    _call_with_optional_deadline(
        ...
        dict(
            ...
            NO_IMPROVEMENT_TIME_LIMIT_SEC=_func1_no_improve_limit_sec(),  # <-- derived from total time
        ),
        deadline_ts=global_deadline,
    )

    _, _, bw_after = bw_from_ranking_path(output_ranking_path)
    # NOTE: typo guard (keep your original EPS name)
    EPS_BW_IMPROVEMENT = EPS_BW_IMPROVE
        ...
        while True:
            ...
            ret = _call_with_optional_deadline(
                ...
                dict(
                    ...
                    NO_IMPROVEMENT_TIME_LIMIT_SEC=_func1_no_improve_limit_sec(),  # <-- derived stall cutoff
                ),
                deadline_ts=global_deadline,
            )
            ...
            if func1_full_every > 0 and (func1_runs % func1_full_every == 0):
                ...
                _phase_log_if_signal(
                    ...
                    bw_before_p,
                    bw_after_p,
                    extra=f"mode=GLOBAL(forced) no_improve_sec={_func1_no_improve_limit_sec():.1f}"
                )
...
graph_file_path = os.path.join(base_dir, "mult16a.d")
