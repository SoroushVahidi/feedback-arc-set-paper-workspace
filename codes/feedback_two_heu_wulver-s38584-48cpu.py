...
def worker_loop(
    ...
    # ---------------- NEW: deadline-aware worker exit ----------------
    ...
    A) Deadline-aware:
       - checks deadline on every queue wakeup and before heavy work
       - if deadline hits, exits cleanly and still sends {"done": True, "stopped_for_deadline": True}

    B) Queue robustness:
       - treats None sentinel as normal exit
       - if deadline hits, does NOT wait for sentinel

    C) Keeps your correctness fixes:
       - strict permutation rank_to_node
       - between via rank_to_node slice
       - changed_scores computed on touched block [idx_v..idx_u] (inclusive)
    """
    import time
    ...
    if debug:
        dbg_log(f"START worker_idx={worker_idx} num_procs={num_procs} QUEUE_GET_TIMEOUT_SEC={QUEUE_GET_TIMEOUT_SEC} deadline_ts={deadline_ts}")
    ...
    while True:
        ...
        t0 = time.time()
        try:
            idx = edge_queue.get(timeout=float(QUEUE_GET_TIMEOUT_SEC))
        ...
        try:
            ...
            if idx_u - idx_v > 1:
                ...
                between = slice_raw  # already no Nones by construction
            else:
                between = []
            ...
            # -----------------------------------------------
            # 3) Extended strategy (skip if deadline is too close)
            # -----------------------------------------------
            if _deadline_hit():
                stopped_for_deadline = True
                break
            ...
            # -----------------------------------------------
            # 6) Accounting + send result
            # -----------------------------------------------
            if success and delta_bw_reduction > 0.0 and changed_scores:
                successes += 1
            ...
            result_queue.put({
                ...
                "u": u,
                "v": v,
                "success": bool(success and delta_bw_reduction > 0.0 and bool(changed_scores)),
                "delta": float(delta_bw_reduction),
                "changed_scores": changed_scores,
                "mode": mode,
                "interval": (lo, hi),  # metadata (may be stale)
                "timing": {
...
def refine_ranking_parallel_dynamic(
    ...
    """
    UPDATED (for fair time-slicing + correct cycle accounting):

    1) Hard respect of `deadline_ts`:
       - frequent deadline checks in: main loop, serial inner loop, worker collect loop.
       - clamps no-improvement timer to remaining time in the slice.

    2) Returns `remaining_pairs_unassessed` for the caller (hybrid driver):
       - When called with `edge_subset`, we track which (u,v) pairs were actually "assessed"
         (included in any DP batch during this call).
       - Return value: (best_scores, final_bw, remaining_pairs_unassessed_list)
         where remaining_pairs_unassessed_list = subset_list - assessed_pairs_in_this_call

    3) Keeps your prior SERIAL fixes:
       - between computed by rank_to_node slicing
       - changed_scores collected via rank_to_node in [lo..hi]
    """
    import time
    import os
    import multiprocessing as mp
    ...
    # Clamp NO_IMPROVEMENT_TIME_LIMIT_SEC to the slice size (prevents 600s hogging inside a 60s slice).
    # Keep at least 1 second so logic doesn't become weird.
    ...
    if edge_subset is not None:
        ...
        for e in edge_subset:
            if isinstance(e, tuple) and len(e) >= 2:
                norm_set.add((e[0], e[1]))
        subset_pairs = norm_set
        subset_list = list(norm_set)

    # Track which subset pairs were assessed in THIS call (for the hybrid driver)
    ...
    while True:
        ...
        if HEARTBEAT_SEC is not None and (now - last_heartbeat_time) >= float(HEARTBEAT_SEC):
            ...
            log_message(
                ...
                f"S={len(S_indices)} left={_time_left():.1f}s",
                log_path
            )
            last_heartbeat_time = now

        # Clamp no-improvement window against remaining time continuously
        ...
        if LOG_EVERY_ROUND and (round_idx % LOG_EVERY_ROUND == 0):
            ...
            log_message(
                ...
                f"| elapsed={elapsed_h:.2f}h since_improve={since_improvement:.1f}s left={_time_left():.1f}s",
                log_path
            )
        ...
        # Mark these pairs as "assessed" for cycle accounting (even if move rejected)
        ...
        if num_edges_batch <= int(SERIAL_BELOW_EDGES):
            ...
            # build rank_to_node once per batch for sorted slicing
            rank_to_node = [None] * n_nodes
            for node, r in scores_snapshot.items():
                rr = int(r)
                if 0 <= rr < n_nodes:
            ...
            for i in range(num_edges_batch):
                ...
                # build `between` in correct rank order
                if idx_v + 1 < idx_u:
                ...
                # collect changed_scores within [lo,hi] using rank_to_node
    ...
    if subset_pairs is not None:
        # We assessed the DP-selected pairs we actually pulled into batches.
        # Everything else in subset_pairs is still unassessed by THIS call.
...
def refine_ranking_parallel_dynamic(
    ...
    """
    Updated implementation-focused version.

    Key updates:
      1) Always returns a tuple with a stable "remaining_pairs_unassessed" contract:
           (best_scores, best_bw, remaining_pairs_unassessed, meta)
         - For GLOBAL (edge_subset=None): remaining_pairs_unassessed = None
         - For TARGETED: remaining_pairs_unassessed = subset_pairs - assessed_pairs_in_this_call

      2) Adds one high-signal return log line:
           [FUNC1][RET] mode=... stop=... subset=... backward0=... assessed=... remaining=...
                         accepted=... attempted=... dp_batches=... dt=... bw: before->after best=...

      3) Tracks practical reject/skip counters so "spinning" becomes visible.

    Existing behavior preserved:
      - Quiet by default (no per-round spam unless LOG_EVERY_ROUND > 0)
      - Crash packets on hard errors (BW increase, mismatch, worker death)
      - SERIAL between-building uses rank_to_node slicing (fast + correct)
    """
    import time
    import os
    import multiprocessing as mp
    ...
    # Mode
    mode = "GLOBAL" if edge_subset is None else "TARGETED"
    ...
    if parallel_overwrites > 0:
        log_message(
            f"[FUNC1][WARN] edges_dict overwrote {parallel_overwrites} parallel arcs (u,v). Consider aggregating.",
            log_path
        )
    ...
    # backward0: how many in subset are backward at START (useful diagnostic)
    backward0 = 0
    if subset_pairs is not None:
        for (u, v, _w) in candidate_edges:
            if scores[u] > scores[v]:
                backward0 += 1

    log_message(
        f"[FUNC1] START pid={pid_main} procs={num_procs} mode={mode} "
        f"| BW={bw0:.2f}/{total_weight:.2f} "
        f"| subset={0 if subset_pairs is None else len(subset_pairs)} backward0={backward0} "
    ...
    # ------------------------------
    # State + Counters (implementation diagnostics)
    # ------------------------------
    best_scores = scores.copy()
    best_BW = float(bw0)
    BW_tracked = float(bw0)

    # Call-scoped accounting for hybrid:
    assessed_pairs = set()        # pairs (u,v) we actually attempted in this call (DP-selected batches)
    attempted_total = 0
    accepted_total = 0
    reject = Counter()            # reasons we skipped/failed
    worker_stats = Counter()      # worker/collect issues
    ...
    while True:
        ...
        if now >= deadline:
            stop_reason = "deadline/time_limit"
            log_message("[FUNC1] STOP deadline/time_limit", log_path)
            break

        if (MAX_DP_BATCHES_PER_CALL is not None) and (dp_batches_done >= MAX_DP_BATCHES_PER_CALL):
            stop_reason = f"max_dp_batches={MAX_DP_BATCHES_PER_CALL}"
            log_message(f"[FUNC1] STOP max_dp_batches={MAX_DP_BATCHES_PER_CALL}", log_path)
            break
        ...
        if HEARTBEAT_SEC is not None and (now - last_heartbeat_time) >= float(HEARTBEAT_SEC):
            ...
            log_message(
                f"[FUNC1] HEARTBEAT BW={BW_tracked:.2f} best={best_BW:.2f} since_improve={since_imp:.1f}s "
                f"S={len(S_indices)} batches={dp_batches_done} attempted={attempted_total} accepted={accepted_total}",
                log_path
            )
            last_heartbeat_time = now
        ...
        if not S_indices:
            ...
            S_indices = rebuild_S_from_scores(scores)
            if not S_indices:
                stop_reason = f"no_backward_edges epoch={epoch_id}"
                log_message(f"[FUNC1] STOP no_backward_edges epoch={epoch_id}", log_path)
                break
        ...
        if LOG_EVERY_ROUND and (round_idx % LOG_EVERY_ROUND == 0):
            ...
            log_message(
                ...
                f"| attempted={attempted_total} accepted={accepted_total}",
                log_path
            )
        ...
        S_indices = filter_S_keep_backward(scores_snapshot, S_indices)
        if not S_indices:
            reject["S_became_empty_after_filter"] += 1
            continue
        ...
        if not edges_for_dp:
            reject["no_edges_for_dp"] += 1
            S_indices = []
            continue

        local_selected = select_nonconflicting_edge_indices_dp(edges_for_dp)
        if not local_selected:
            reject["dp_selected_empty"] += 1
            S_indices = []
            continue
        ...
        dp_batches_done += 1
        attempted_total += num_edges_batch
        for (u, v, _w, _lo, _hi) in batch_edges:
            assessed_pairs.add((u, v))
        ...
        if num_edges_batch <= int(SERIAL_BELOW_EDGES):
            ...
            for i in range(num_edges_batch):
                ...
                try:
                    ...
                    if ext_success and float(ext_bw_reduction) > 0.0:
                        ...
                        reject["serial_ext_nonpos"] += 1
                except Exception:
                    reject["serial_ext_error"] += 1
                    success = False
                    delta = 0.0
                    mode_used = "extended_error"
                ...
                if not success:
                    try:
                        ...
                        if best_gain > 0.0:
                            ...
                            if best_gain == float(g1):
                                scores_local[u], scores_local[v] = idx_v, idx_u
                                mode_used = "greedy/swap"
                            elif best_gain == float(g2):
                                for k, r in list(scores_local.items()):
                                    if idx_v < r <= idx_u:
                                        scores_local[k] = r - 1
                                scores_local[v] = idx_u
                                mode_used = "greedy/move_v_after_u"
                            else:
                                for k, r in list(scores_local.items()):
                                    if idx_v <= r < idx_u:
                                        scores_local[k] = r + 1
                                scores_local[u] = idx_v
                                mode_used = "greedy/move_u_before_v"
                        else:
                            reject["serial_greedy_nonpos"] += 1
                    except Exception:
                        reject["serial_greedy_error"] += 1
                        success = False
                        delta = 0.0
                        mode_used = "greedy_error"

                if not (success and delta > EPS_IMPROVE):
                    reject["serial_no_accept"] += 1
                    continue

                # collect changed_scores within [lo,hi] using rank_to_node range
                lo_i = int(lo) if lo is not None else 0
                hi_i = int(hi) if hi is not None else (n_nodes - 1)
                lo_i = max(0, lo_i)
                hi_i = min(n_nodes - 1, hi_i)
                if lo_i > hi_i:
                    reject["serial_bad_interval"] += 1
                    continue
                ...
                if not changed_scores:
                    reject["serial_empty_changed_scores"] += 1
                    continue
                ...
                if recompute_left > 0:
                    ...
                    if bw_after > bw_before + BW_INCREASE_TOL:
                        _log_crash_packet(
                            ...
                            num_edges_batch=num_edges_batch,
                            extra_lines=[
                                f"path=SERIAL i={i}/{num_edges_batch} mode={mode_used} u={u} v={v} w={w} idx_u={idx_u} idx_v={idx_v} stored(lo,hi)=({lo},{hi}) clamp(lo,hi)=({lo_i},{hi_i})",
                                f"bw_before={bw_before:.6f} bw_after={bw_after:.6f} delta_reported={delta:.6g} bw_delta_recomp={bw_after-bw_before:.6g}",
                                _summarize_changed_scores(changed_scores, scores),
                                f"between_len={len(between)} between_sorted=YES (rank_to_node slice)",
                            ]
                ...
                if BW_tracked > bw_before_tracked + BW_INCREASE_TOL:
                    _log_crash_packet(
                        ...
                        num_edges_batch=num_edges_batch,
                        extra_lines=[
                            f"path=SERIAL i={i}/{num_edges_batch} mode={mode_used} u={u} v={v} w={w} idx_u={idx_u} idx_v={idx_v} delta_reported={delta:.6g}",
                            f"BW_tracked_before={bw_before_tracked:.6f} BW_tracked_after={BW_tracked:.6f} increase={BW_tracked-bw_before_tracked:.6g}",
                            _summarize_changed_scores(changed_scores, scores_snapshot),
                        ]
                    )
            ...
            # sanity check sometimes (log only on error)
            if BW_CHECK_EVERY_ROUND > 0 and (round_idx % BW_CHECK_EVERY_ROUND == 1):
                fw_recompute = compute_forward_weight(edges, scores)
                bw_recompute = total_weight - float(fw_recompute)
                if abs(bw_recompute - BW_tracked) > BW_MISMATCH_TOL:
            ...
            if (time.time() - last_improvement_time) >= NO_IMPROVEMENT_TIME_LIMIT_SEC:
                stop_reason = f"no_improve_for={time.time()-last_improvement_time:.1f}s limit={NO_IMPROVEMENT_TIME_LIMIT_SEC:.0f}s"
                log_message(
        ...
        for wi in range(num_procs_effective):
            p = mp.Process(
                ...
                kwargs={
                    ...
                    "DEBUG_LOG_TOP_GAINS": bool(WORKER_DEBUG_LOGS),
                    "DEBUG_BIG_GAIN": float(WORKER_DEBUG_BIG_GAIN),
                    "deadline_ts": float(deadline),   # <<< NEW
                }
            )
            p.daemon = False
            p.start()
        ...
        while alive_workers > 0:
            if time.time() >= deadline:
                broke_for_deadline = True
                worker_stats["deadline_during_collect"] += 1
                log_message(f"[FUNC1][WARN] STOP during_collect deadline batch={dp_batches_done}", log_path)
                break
            ...
            try:
                ...
                if all((not p.is_alive()) for p in workers):
                    worker_stats["all_workers_dead_before_done"] += 1
                    _log_crash_packet(
                        "ALL_WORKERS_DEAD_BEFORE_DONE",
                        batch=dp_batches_done,
                        round_idx=round_idx,
                    ...
                    log_message(f"[FUNC1][WARN] all_workers_dead_before_done batch={dp_batches_done}", log_path)
                    break
                worker_stats["result_queue_timeout"] += 1
                continue
        ...
        if len(worker_done) < num_procs_effective:
            ...
            worker_stats["missing_done_msgs"] += len(missing)
            log_message(
        ...
        if broke_for_deadline:
            stop_reason = "deadline/time_limit (during_collect)"
            log_message("[FUNC1] STOP deadline/time_limit", log_path)
            break
        ...
        for r in results:
            ...
            delta = float(r.get("delta", 0.0) or 0.0)
            changed_scores = r.get("changed_scores", {}) or {}
            mode_used = r.get("mode", "?")
            interval = r.get("interval", None)
            u = r.get("u", None)
            v = r.get("v", None)

            if not sflag:
                reject["parallel_worker_fail"] += 1
                continue
            if not (delta > EPS_IMPROVE):
                reject["parallel_nonpos_delta"] += 1
                continue
            if not changed_scores:
                reject["parallel_empty_changed_scores"] += 1
                continue
            ...
            if recompute_left > 0:
                ...
                if bw_after > bw_before + BW_INCREASE_TOL:
                    _log_crash_packet(
                        ...
                        num_edges_batch=num_edges_batch,
                        extra_lines=[
                            f"path=PARALLEL mode={mode_used} u={u} v={v} interval={interval} delta_reported={delta:.6g}",
                            f"bw_before={bw_before:.6f} bw_after={bw_after:.6f} bw_delta_recomp={bw_after-bw_before:.6g}",
                            _summarize_changed_scores(changed_scores, scores),
                        ]
                    )
    ...
    # remaining pairs contract for hybrid
    remaining_pairs_unassessed = None
    subset_n = 0 if subset_pairs is None else len(subset_pairs)
    if subset_pairs is not None:
        # "assessed" means: DP-selected edges that we actually attempted in this call.
        # Remaining = subset - assessed (regardless of forward/backward status now).
        remaining_pairs_unassessed = set(subset_pairs) - set(assessed_pairs)

    dt = time.time() - t_start

    # One-line return summary (high signal)
    log_message(
        f"[FUNC1][RET] mode={mode} stop={stop_reason} "
        f"subset={subset_n} backward0={backward0} assessed={len(assessed_pairs)} remaining={0 if remaining_pairs_unassessed is None else len(remaining_pairs_unassessed)} "
        f"attempted={attempted_total} accepted={accepted_total} dp_batches={dp_batches_done} "
        f"dt={dt:.2f}s | BW {bw0:.6f}->{final_bw:.6f} best={best_BW:.6f}",
        log_path
    )

    # Optional: compact reject summary only if things look “suspicious”
    # (e.g., many attempts but zero accepts)
    if attempted_total > 0 and accepted_total == 0:
        top_rej = reject.most_common(8)
        top_wrk = worker_stats.most_common(8)
        log_message(f"[FUNC1][RET][REJECT_TOP] {top_rej}", log_path)
        if top_wrk:
            log_message(f"[FUNC1][RET][WORKER_TOP] {top_wrk}", log_path)

    meta = {
        "mode": mode,
        "stop_reason": stop_reason,
        "bw_start": float(bw0),
        "bw_end": float(final_bw),
        "best_bw_seen": float(best_BW),
        "subset_size": int(subset_n),
        "backward0": int(backward0),
        "assessed_pairs": int(len(assessed_pairs)),
        "remaining_pairs": None if remaining_pairs_unassessed is None else int(len(remaining_pairs_unassessed)),
        "attempted_total": int(attempted_total),
        "accepted_total": int(accepted_total),
        "dp_batches_done": int(dp_batches_done),
        "runtime_sec": float(dt),
        "reject_top": reject.most_common(15),
        "worker_top": worker_stats.most_common(15),
    }

    return best_scores, final_bw, remaining_pairs_unassessed, meta
...
def parallel_refine_largest_scc_intervals(
    ...
    ML logging:
      - If ml_csv_path is not None, appends 1 row per applied block using ml_append_row(...).
      - Label is local_delta_fw (worker estimate) and batch-level exact ΔFW is also logged.
    """
    ...
    # ---------------- ML helper ----------------
    def _ml_row_base(**kw):
        # Keep it flat + robust; never prints
        row = {
            "phase": "func2",
            "ts": time.time(),
            "batch": int(batch_idx),
    ...
    while True:
        ...
        # NEW: block_id -> meta (for ML logging)
        block_meta = {}
        ...
        for res in results:
            ...
            if n_block < 2 or not block_ranks:
                # Optional ML: log that worker produced empty proposal
            ...
            if local_delta_fw < -float(eps_bw_increase):
                ...
                # ML: log the violating proposal before crash
                if ml_csv_path is not None:
                    ml_append_row(
                        ml_csv_path,
                        _ml_row_base(
            ...
            # ML: log applied block (label = local_delta_fw)
            if ml_csv_path is not None:
                ml_append_row(
                    ml_csv_path,
                    _ml_row_base(
        ...
        # ML: optionally log a batch summary row
        if ml_csv_path is not None:
            ml_append_row(
                ml_csv_path,
                _ml_row_base(
...
def hybrid_refine_func1_func2(
    ...
    UPDATED: Fair time distribution between func1 and func2:
      - Both func1 and func2 are time-sliced (deadline_ts = now + SLICE_SEC).
      - The scheduler picks the next phase based on cumulative time spent vs target fraction.
      - func2 is no longer starved by "only-on-empty" gating; empty-cycle func2 still happens,
        but the fairness scheduler can also run func2 even when cycle isn't empty.
    """
    import os
    import time
    import shutil
    ...
    def _pick_next_phase(cycle_remaining_nonempty: bool) -> str:
        ...
        We try to keep: time_spent_func1 / (time_spent_func1 + time_spent_func2) ~= FUNC1_TARGET_TIME_FRACTION.
        """
        ...
        # If func1 is ahead of its target share, favor func2; if behind, favor func1.
    ...
    # fairness counters (wall-clock time actually spent inside the calls)
    ...
    # =====================================================
    # PHASE 1: func1 GLOBAL (initial)  -- NOW time-sliced
    # =====================================================
    phase_idx += 1
    func1_runs += 1
    t0 = time.time()
    ...
    f1_deadline = min(global_deadline, time.time() + float(FUNC1_SLICE_SEC))
    ...
    try:
        while True:
            ...
            # Always keep cycle up-to-date if empty
            ...
            # If we just refilled and it’s empty -> nothing to do (handled above).
            cycle_nonempty = len(cycle_remaining) > 0

            # Optional legacy behavior: brief func2 burst on empty cycle (kept)
            # NOTE: with fairness scheduling, func2 can also run even when cycle_nonempty.
            if (not cycle_nonempty) and _sec_left() >= float(FUNC2_ON_EMPTY_MIN_SEC_LEFT) and float(FUNC2_ON_EMPTY_SEC) > 0.0:
                phase_idx += 1
                func2_runs += 1
                t0 = time.time()
                ...
                # after func2, refresh cycle next iteration
            ...
            # ------------------------------
            next_phase = _pick_next_phase(cycle_remaining_nonempty=(len(cycle_remaining) > 0))
            ...
            if next_phase == "func2":
                ...
                _, _, bw_before2 = bw_from_ranking_path(current_input_path)
                tmp_deadline = min(global_deadline, time.time() + float(FUNC2_SLICE_SEC))
                ...
                # IMPORTANT: after func2, your backward-edge set changes; force a refresh.
                cycle_remaining.clear()
            ...
            # ------------------------------
            # Run func1 (targeted or global) (sliced)
            # ------------------------------
            else:
                ...
                if not do_global:
                    # targeted work on cycle_remaining
...
graph_file_path = os.path.join(base_dir, "s38584.d")
